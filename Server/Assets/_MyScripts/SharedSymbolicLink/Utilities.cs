...
public static class Utilities {
	...
	public enum StringTpye : byte {
		Undefined, Email, Username, UsernameAndDiscriminator,
	}

	public static StringTpye TestString(string test) {
		if (IsEmail(test)) {
			return StringTpye.Email;
		} else if (IsUsername(test)) {
			return StringTpye.Username;
		} else if (IsUsernameAndDiscriminator(test)) {
			return StringTpye.UsernameAndDiscriminator;
		}
		return StringTpye.Undefined;
	}
}
